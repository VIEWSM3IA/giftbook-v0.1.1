# V0.2 匿名真实案例验收 · 2026-09-25

- `npm test`：28 项通过。覆盖旧数据迁移、公开案例授权与匿名字段、筛选、审核拦截、重复「有帮助」、编辑和下架、删除私有记录后的自动下架、小程序表单提交。
- `npm run check`：8 个原生页面和组件文件、模板事件、JS/JSON 语法及双底部入口检查通过。
- 隔离 PostgreSQL schema 上的 `npm run check:h5`：完整移动浏览器流程通过，含分享公开预览、第二账号浏览与有帮助、筛选、作者编辑下架、原 V0.1.1 流程；320/390/430px 无横向溢出，浏览器无 pageerror。截图在 `/tmp/giftbook-v02-h5/`，测试账号已注销。
- 固定公网验收入口上同一套 `npm run check:h5` 通过，截图在 `/tmp/giftbook-v02-public/`，测试账号已注销。预览服务已重启并自动执行可重复迁移；迁移前数据库备份为 `/tmp/giftbook-v02-predeploy-20260925.dump`（权限 `0600`）。
- 原生微信小程序结构与逻辑已验证；微信开发者工具编译、真实微信授权及实体设备触控未验证。基础审核是规则检查，不替代人工内容审核。
