...
(async () => {
  ...
  try {
    ...
    try {
      ...
      const sharedCard = viewerPage.locator('.case-card').filter({ has: viewerPage.getByRole('heading', { name: '公开版拍立得', exact: true }) });
      await sharedCard.getByRole('button', { name: '有帮助 · 0' }).click();
      await sharedCard.getByRole('button', { name: '已觉得有帮助 · 1' }).waitFor();
